...
foreach ($years as $year) {
    ...
    $struct[$year] = array();
    $monthes = scandir("img/{$main_dir}/{$year}");
    foreach ($monthes as $month) {
        if ($month == '.' || $month == '..') continue;
        $struct[$year][$month] = $month;
        $photos = scandir("img/{$main_dir}/{$year}/{$month}");
        natsort($photos);
        foreach ($photos as $photo) {
            if ($photo == '.' || $photo == '..' || strpos($photo, '_m') > 0) continue;
            $hod_photos["{$year}_{$month}"][] = "img/{$main_dir}/{$year}/{$month}/{$photo}";
        }
    }
}
//arsort($struct);
